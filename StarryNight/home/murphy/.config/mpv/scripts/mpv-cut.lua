...
ACTIONS.ENCODE = function(d)
	local formatted_duration = string.format("%.2f", d.duration)
	print("To " .. Formatted_start_time + formatted_duration .. "s")
	local args = {
		"ffmpeg",
		"-nostdin", "-y",
		"-loglevel", "error",
		...
		"-crf", "16",
		"-preset", "superfast",
		utils.join_path(d.indir, "Cutted_" .. d.infile_noext .. d.ext)
	}
	mp.command_native_async({
		name = "subprocess",
		args = args,
...
local function text_overlay_on()
	...
	Formatted_start_time = string.format("%.2f", START_TIME)
	print("Cutting from: " .. Formatted_start_time .. "s")
